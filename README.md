# Digital-Literacy-Project

## Student Information

Name: Protyush Mitra 

Registration Number: 25BAI10024 

Program: B.Tech CSE (AIML) 

University: VIT Bhopal University 

Batch: 2025–2029 

## Project Overview

This repository contains the completed tasks for the Digital Literacy Project (Project 1). As a Student Digital Ambassador, I have developed a suite of digital assets and professional profiles to demonstrate my proficiency in modern digital tools, professional communication, and cybersecurity awareness. 

## Repository Structure



### task-1-presentation:
Contains the Digital Literacy Infographic (Purple/AI themed).

### task-2-portfolio: 
Contains professional profiles for LinkedIn and the GitHub README.

### task-3-platforms: 
Proof of coding skills (HackerRank Python Badge) and the Digital Literacy Awareness Google Form.

### task-4-email-etiquette: 
"Professional email drafts and a social media ""Do's and Don'ts"" checklist."

### task-5-cybercrime: 
A detailed case study on Phishing and an 8-point prevention checklist.

### report: 
The final consolidated Project Report in PDF format.

## Technical Skills Demonstrated

Graphic Design: Canva (Infographic Creation).

Version Control: GitHub (Repository Management & Profile README).

Programming: Python (HackerRank Skills Verification).

Collaboration Tools: Google Workspace (Forms & Sheets).

Professionalism: Formal Email Writing & Digital Etiquette.

## Submission Status

Deadline: March 31, 2026.Status: All tasks completed and documented for final evaluation.





